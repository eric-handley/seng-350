...
// --- Minimal smoke tests (run in dev only) ---
function runSmokeTests() {
  try {
    console.assert(SAMPLE_AUDITS.length === 42, "Audit sample length");
    const onlyHigh = filterAudits(SAMPLE_AUDITS, "", "high");
    console.assert(
      onlyHigh.every((r) => r.severity === "high"),
      "Severity filter"
    );
    const q = SAMPLE_AUDITS[0].actor.split("@")[0];
    const qRes = filterAudits(SAMPLE_AUDITS, q, "all");
    console.assert(qRes.length > 0, "Text query finds some rows");
  } catch (e) {
